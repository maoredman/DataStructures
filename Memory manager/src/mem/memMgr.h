...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      ret = (T*)_ptr; //should ret be updated even if false?
   ...
   // Data members
   char*             _begin;
   char*             _ptr; //points to memory about to be written
   char*             _end; //points to memory with nothing in it yet
...
class MemRecycleList
{
   ...
   T* popFront() { //used for alloc()
      // TODO
      T* old_first = _first;
      if(_first != 0)//there is a next element
         _first = getNext(_first); //*_first is memory address of next block
      return old_first;
      //returns old block's pointer
   }
   // push the element 'p' to the beginning of the recycle list
   void  pushFront(T* p) { //used for delete()
      // TODO
      *(size_t*)p = (size_t)_first; //leading content of block will be address of next block
      _first = p;
   }
   ...
   void reset() {
      ...
      while(_nextList != 0){
         _nextList->reset();
         //cout<<"gonna delete nextlist"<<endl;
         delete _nextList;
         _nextList = 0;
      }
      _first = 0; //don't need to delete _first, as it points to memory already deleted
   }
   ...
   T* getNext(T* p) const {
      // TODO
      if(p == 0)
         return 0; //otherwise segmentation fault
      return (T*) *(size_t*)p;
   }
   //
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      _activeBlock->reset(); //Block reset, not Mgr reset (this function)!
      //cout<<"up to here ok"<<endl;
      for(size_t i = 0; i < R_SIZE; ++i){
         _recycleList[i].reset();
      }
      //cout<<"up to here NOT ok"<<endl;
      if(b != 0){
         _blockSize = b;
         delete _activeBlock;
         _activeBlock = new MemBlock<T>(0, _blockSize);
...
private:
   ...
   // [Note] t must >= S, where S is sizeof(T)
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* current_Recycle_List = &_recycleList[m];
      while(current_Recycle_List->getArrSize() != n){
         if(current_Recycle_List->_nextList == 0){
            current_Recycle_List->_nextList = new MemRecycleList<T>(n);
         }
         current_Recycle_List = current_Recycle_List->_nextList;
      }
      return current_Recycle_List;
   }
   ...
   T* getMem(size_t t) {
      ...
      #endif // MEM_DEBUG
      size_t nbytes = toSizeT(t);
      if(nbytes > _blockSize){
         cerr << "Requested memory (" << t << ") is greater than block size" << "(" << _blockSize << "). " << "Exception raised...\n";
         throw bad_alloc();
      }

      size_t rn = getArraySize(nbytes);

      ret = getMemRecycleList(rn)->popFront(); //if getArraySize was not already in MemRecycleList, a new MemRecycleList with _first = 0 is created
      #ifdef MEM_DEBUG
         if(ret != 0)
            cout << "Recycled from _recycleList[" << rn << "]..." << ret << endl;
      #endif // MEM_DEBUG

      if(ret == 0){
         if(!_activeBlock->getMem(nbytes, ret)){ //ret now points to (T*)_ptr
            //not enough memory in _activeBlock
            size_t bytes_left = _activeBlock->getRemainSize();
            if(bytes_left >= toSizeT(S)){
               size_t array_size_left = getArraySize(bytes_left);
               getMemRecycleList(array_size_left)->pushFront(ret);//pushing (T*)_ptr
               #ifdef MEM_DEBUG
                  cout << "Recycling " << ret << " to _recycleList[" << array_size_left << "]\n";
               #endif // MEM_DEBUG
            }

            _activeBlock = new MemBlock<T>(_activeBlock, _blockSize);
            _activeBlock->getMem(nbytes,ret);
            #ifdef MEM_DEBUG
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t count = 1;
      MemBlock<T>* _tempBlock = _activeBlock;
      while(_tempBlock->_nextBlock != 0){
         ++count;
         _tempBlock = _tempBlock->_nextBlock;
      }
      return count;
   }
