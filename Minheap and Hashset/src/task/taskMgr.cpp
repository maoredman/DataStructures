...
: _taskHeap(nMachines), _taskHash(getHashSize(nMachines)) { } //why is getHashSize defined like that?
...
TaskMgr::add(size_t nMachines)
{
  ...
  for(size_t i = 0; i < nMachines; i++){
    TaskNode newNode = TaskNode();
    while(!_taskHash.insert(newNode)) { newNode = TaskNode(); }
...
{
   // TODO...
  if(taskMgr->empty()) { return false; }
  TaskNode newNode = TaskNode(_taskHeap.min().getName(), _taskHeap.min().getLoad() + l);
  _taskHeap.delMin();
  _taskHeap.insert(newNode);
